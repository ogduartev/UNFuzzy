...
wxString SistemaLogicaDifusa::crearEncabezadoTabla(wxString nombreArchivo)
{
	wxString nomArchivo;
	nomArchivo = wxFileName::FileName(nombreArchivo).GetFullName();
	...
	wxString str="";
	str << "# " << _("Data file generated by UNFUZZY") << "\n";
	str << "# " << _("You can visualize the data surfaces using gnuplot.") << "\n";
	str << "# " << _("Use, for example, the following instructions in gnuplot:") << "\n";
	str << "# \n";
 	str << "# " <<   "set title '" << nomArchivo << "'" << "\n";
	if(numEntradas==1)
	{
  	str << "# " << "set xlabel '" << variableEntrada(0)->nombreVariable() << "'\n";
	  str << "# " << "set ylabel '" << variableSalida(0)->nombreVariable()  << "'\n";
	  str << "# " << "plot '" << nomArchivo << "' u 1:2 with lines notitle\n";
	}else
	{
		str << "# " << "set xyplane at " << variableSalida(0)->rangoMinimo() << "\n";
  	str << "# " << "set xlabel '" << variableEntrada(0)->nombreVariable() << "'\n";
  	str << "# " << "set ylabel '" << variableEntrada(1)->nombreVariable() << "'\n";
	  str << "# " << "set zlabel '" << variableSalida(0)->nombreVariable()  << "'\n";
	  str << "# " << "splot '" << nomArchivo << "' u 1:2:" << (entradas->numeroVariables()+1) << " with lines palette notitle\n";
	}
	str << "# ";
	return str;
}
...
/////////////////////////
	...
	linea=crearEncabezadoTabla(nombreArchivo);
  ...
  linea = _("# ");
	...
	{
		linea << "'" << nombreVariableEntrada(i) << "'\t";
	}
	for(i=0;i<numSalidas;i++)
	{
		linea << "'" << nombreVariableSalida(i) << "'\t";
	}
	tablaFile.AddLine(linea);
	...
	while(bandera!=0)
	{
		...
		tablaFile.AddLine(linea);
		if(numEntradas>1 && contadorIntervalos[numEntradas-1]==0)
		{
  		linea="";
	  	tablaFile.AddLine(linea);
		}
...
void SistemaLogicaDifusa::generarCodigoCPPClase(wxTextFile* codigoFile,wxString nombreClase)
{
	...
	Codigo=("    Norma *MinComposicion;");codigoFile->AddLine(Codigo);
	Codigo=("    Norma *MaxComposicion;");codigoFile->AddLine(Codigo);
	...
	Codigo=("    MinComposicion=new ");
	Codigo << concreto->motor()->minComposicion()->codigoCPP();codigoFile->AddLine(Codigo);
	Codigo=("    MaxComposicion=new ");
	Codigo << concreto->motor()->maxComposicion()->codigoCPP();codigoFile->AddLine(Codigo);
//////////////
	Codigo=("    Implica=new ");
	Codigo << concreto->motor()->implicacion()->codigoCPP();codigoFile->AddLine(Codigo);
	Codigo=("    motor->and_(And);");codigoFile->AddLine(Codigo);
	Codigo=("    motor->minComposicion(MinComposicion);");codigoFile->AddLine(Codigo);
	Codigo=("    motor->maxComposicion(MaxComposicion);");codigoFile->AddLine(Codigo);
...
void SistemaLogicaDifusa::generarCodigoCSistema(wxTextFile* codigoFile)
{
	...
	Codigo="}\r\n";codigoFile->AddLine(Codigo);
	Codigo="double minComposicion(double x,double y)";codigoFile->AddLine(Codigo);
	Codigo="{";codigoFile->AddLine(Codigo);
	Codigo="    double z;";codigoFile->AddLine(Codigo);
	Codigo=concreto->motor()->minComposicion()->codigoC();codigoFile->AddLine(Codigo);
	Codigo="    return z;";codigoFile->AddLine(Codigo);
	Codigo="}\r\n";codigoFile->AddLine(Codigo);
	Codigo="double maxComposicion(double x,double y)";codigoFile->AddLine(Codigo);
	Codigo="{";codigoFile->AddLine(Codigo);
	Codigo="    double z;";codigoFile->AddLine(Codigo);
	Codigo=concreto->motor()->maxComposicion()->codigoC();codigoFile->AddLine(Codigo);
	Codigo="    return z;";codigoFile->AddLine(Codigo);
	Codigo="}\r\n";codigoFile->AddLine(Codigo);
	Codigo="double Conjuncion(double x,double y)";codigoFile->AddLine(Codigo);
	Codigo="{";codigoFile->AddLine(Codigo);
